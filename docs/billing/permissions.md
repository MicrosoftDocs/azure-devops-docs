...
but by membership in an access level. To learn more, see [Access levels](../security/access-levels.md). <!--- Manage users and access (Team Services)](add-account-users-assign-access-levels-team-services.md) or [Change access levels (TFS)](../work/connect/change-access-levels.md). --> 
...
<tbody valign="top">
	...
	<tr>
		...
		<td>
			...
			<blockquote>
				If your deployment uses SharePoint or Reporting,
				consider [adding the members of this group](add-administrator-tfs.md#project-collection)
...
You manage server-level permissions through the [Team Foundation Administration Console](add-administrator-tfs.md) or [TFSSecurity command-line tool](../setup-admin/tfs/command-line/tfssecurity-cmd.md#permissions). Team Foundation Administrators are granted all server-level permissions. Other server-level groups have select permission assignments.
...
You manage collection-level permissions through the [web portal admin context](../connect/work-web-portal.md#admin-context) or [TFSSecurity command-line tool](../setup-admin/tfs/command-line/tfssecurity-cmd.md#collection-level-permissions). Project Collection Administrators are granted all collection-level permissions. Other collection-level groups have select permission assignments.
...
<tbody valign="top">
	...
	<tr>
		...
		<td>
			...
			<blockquote>
				...
				[Create a New Team Project Wizard](create-team-project.md).
			</blockquote>
		</td>
	</tr>
	<tr>
	...
	<tr>
		...
		<td>
			Can [delete team projects](delete-team-project.md).
			<blockquote>
...
You manage project-level permissions from the [web portal admin context](../connect/work-web-portal.md#admin-context) or using the [TFSSecurity command-line tool](../setup-admin/tfs/command-line/tfssecurity-cmd.md#team-project-level-permissions). Project Administrators are assigned all project-level permissions. Other project-level groups are assigned a subset of these permissions.
...
<tbody valign="top">
	...
	<tr>
		<td id="delete-team-project-permission">Delete team project</td>
		<td>Can [delete the team project](delete-team-project.md) from the collection.</td>
	</tr>
	<tr>
	...
	<tr>
		...
		<td>Can [change the name of the team project](rename-team-project.md).</td>
	</tr>
	<tr>
...
You manage build permissions [for each build defined in the web portal](../build-release/concepts/policies/permissions.md#build-permissions) or using the [TFSSecurity command-line tool](../setup-admin/tfs/command-line/tfssecurity-cmd.md#build-permissions). Project Administrators are granted all build permissions and Build Administrators are assigned most of these permissions. You can set build permissions for each build definition.
...
You manage the security of each Git repository or branch from the [web portal](../git/branch-permissions.md) or using the [TFSSecurity command-line tool](../setup-admin/tfs/command-line/tfssecurity-cmd.md#git-repo-permissions). Project Administrators are granted most of these permissions (which appear only for a team project that's been configured with a Git repository). You can manage these permissions for each Git repository.
...
-->
	<tr>
		...
		<td><p>At the repository level, can push their changes to existing branches in the repository. Users who lack this permission but who have [create branch](#git-create-branch-permission) may push changes to new branches. Does not override restrictions in place from [branch policies](../git/branch-policies.md).</p>
...
You manage the security of each TFVC branch from the [web portal](restrict-access-tfs.md) or using the [TFSSecurity command-line tool](../setup-admin/tfs/command-line/tfssecurity-cmd.md#tfvc-permissions). Project Administrators are granted most of these permissions which appear only for a team project that's been configured to use Team Foundation Version Control as the source control system. In version control permissions, explicit deny takes precedence over administrator group permissions.
...
You manage the security of each iteration path from the [web portal](../work/how-to/set-permissions-access-work-tracking.md) or using the [TFSSecurity command-line tool](../setup-admin/tfs/command-line/tfssecurity-cmd.md#iteration-permissions). 
...
You manage tagging permissions mostly from the [TFSSecurity command-line tool](../setup-admin/tfs/command-line/tfssecurity-cmd.md#tagging-permissions). Contributors can add tags to work items and use them to quickly filter a backlog, board, or query results view.
		...
		<td>
			...
			<blockquote>
				<p>
					...
					It can only be set by using the [TFSSecurity](../setup-admin/tfs/command-line/tfssecurity-cmd.md) command.
				</p>
				<p>
...
You can set these permissions by using the [TFSLabConfig permissions command-line tool](../setup-admin/tfs/command-line/tfslabconfig-cmd.md#permissions).
...
There are no UI permissions associated with [managing email notifications or alerts](../work/track/alerts-and-notifications.md). Instead, they can be managed using the [TFSSecurity command line tool](../setup-admin/tfs/command-line/tfssecurity-cmd.md#collection-level-permissions).
...
You can manage alert permissions using [TFSSecurity](../setup-admin/tfs/command-line/tfssecurity-cmd.md).
...
|---|---|---|:---:|
|CREATE_SOAP_SUBSCRIPTION|EventSubscription|Can create a SOAP-based web service subscription.|![](../_img/check.png)|
|GENERIC_READ|EventSubscription|Can view subscription events defined for a team project.|![](../_img/check.png)|
|GENERIC_WRITE|EventSubscription|Can create alerts for other users or for a team.|![](../_img/check.png)|
|UNSUBSCRIBE|EventSubscription|Can unsubscribe from an event subscription.|![](../_img/check.png)|
...
- [Add users to a team project](add-team-members-vs.md) (Team Services)   
