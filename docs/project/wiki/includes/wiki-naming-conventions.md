...
- **File name**: When you choose a name for the page file, remember that the fully qualified path to the file should be 235 characters or less. The full path to the page consists of the repo URL, the folder path to the page file, and the page file name. For example, `https://github.com/ExampleWiki/Contributors/Code/How-to-add-code-to-the-project.md`.
...
- **Special characters**: The page file name has restrictions on the following special characters:
   - No Unicode control characters or surrogate characters
   - No printable characters: forward slash `/`, backslash `\`, hash `#`
...
A page file name can include several special characters, such as the colon `:` and hyphen `-`. For example, you might name a Markdown file as _FAQ:0525_ or _Setup-Guide_. 
