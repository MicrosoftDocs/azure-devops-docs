...
Learn how to install, assign, disable, and uninstall extensions for Azure DevOps. Extensions add new features and capabilities to your organization.

For information about building your own, see the [developing](../extend/overview.md) and [publishing](../extend/publish/overview.md) overviews.
...
|--|--|
| **Permissions** | Member of the [Project Collection Administrators](../organizations/security/look-up-project-collection-administrators.md) group. Organization owners are automatically members of this group. If you don't have permissions, you can [request extensions](./request-extensions.md) instead. |
| **Private extensions** | The extension must be [shared with your organization](../extend/publish/overview.md#share-your-extension). |
...
| Category | Requirements |
|--|--|
| **Permissions** | Member of the [Project Collection Administrators](../organizations/security/look-up-project-collection-administrators.md) group or have **Edit collection-level information** permissions. Organization owners are automatically members of this group. If you don't have permissions, you can [request extensions](./request-extensions.md) instead. |
| **Private extensions** | The extension must be [shared with your organization](../extend/publish/overview.md#share-your-extension). |
| **Billing** | To use an existing Azure subscription for billing, you need at least coadministrator permissions for that subscription. If you don't have permissions, an Azure Account Administrator or Service Administrator can [add you as coadministrator](/azure/billing-add-change-azure-subscription-administrator) in the Azure portal. Your project collection reuses your Azure subscription for future Marketplace purchases. |
...
1. Find the extension that you want to install and select **Get it free**.

    ![Screenshot showing red square highlighting Get it free button.](media/get-vsts-extensions/get-extension.png)

1. Select your organization from the dropdown menu, and then select **Install**.

    ![Screenshot showing organization dropdown and Install button for the extension.](media/get-vsts-extensions/select-install-extension.png)

    > [!TIP]
    > If you don't see your organization or can't install the extension, see [Frequently asked questions](#frequently-asked-questions-faqs).

After installation completes, go to your organization to start using the extension.
...
Use the [az devops extension install](/cli/azure/devops/extension#az-devops-extension-install) command to install an extension. To get started, see [Get started with Azure DevOps CLI](../cli/index.md).

### Search for an extension

If necessary, first search for an extension by using the `az devops extension search` command.
...
- **org**: Azure DevOps organization URL. You can configure the default organization by using `az devops configure -d organization=ORG_URL`. If you don't configure a default organization or if `git config` doesn't pick up an organization, you need to provide this parameter. Example: `--org https://dev.azure.com/MyOrganizationName/`.
...
1. Replace `<extension-name>` with the actual name of the extension you want to install.
...
Extensions with high privilege scopes, pipeline decorators, or unpublished status can pose potential security risks if not properly vetted. High privilege scopes grant extensive access to your organization's resources, while pipeline decorators can modify all pipelines in your organization. Publishers might no longer maintain unpublished extensions. For more information on managing these types of extensions, see [Manage high privilege scopes, pipeline decorators, and unpublished extensions](manage-high-privilege-extensions.md).
...
1. Select **Extensions**, and then select the extension that you want to uninstall or disable.

   ![Screenshot showing Select uninstall or disable for extension.](media/org-settings-select-extension.png)

1. Select **Uninstall** or select the ellipses (**...**), and then select **Disable**.
...
Use the [az devops extension uninstall](/cli/azure/devops/extension#az-devops-extension-uninstall) command to uninstall an extension. For more information, see [Get started with Azure DevOps CLI](../cli/index.md).
...
- **org**: Azure DevOps organization URL. Configure the default organization by using `az devops configure -d organization=ORG_URL`. If you don't configure the default organization, you must specify this parameter. `git config` can also provide the organization. Example: `--org https://dev.azure.com/MyOrganizationName/`.
...
Use the [az devops extension disable](/cli/azure/devops/extension#az-devops-extension-disable) command to disable an extension. For more information, see [Get started with Azure DevOps CLI](../cli/index.md).
...
To uninstall extensions in a collection, complete the following steps: 

1. Go to the local gallery management portal (`http://{server}:8080/tfs/_gallery/manage`).

1. For the extension you want, select the ellipses (**...**), and then select **Remove**.
...
Enable an extension by using the [az devops extension enable](/cli/azure/devops/extension#az-devops-extension-enable) command. To get started, see [Get started with Azure DevOps CLI](../cli/index.md).
...
Use the [az devops extension list](/cli/azure/devops/extension#az-devops-extension-list) command to list the extensions that are installed in your organization. To get started, see [Get started with Azure DevOps CLI](../cli/index.md).
...
Use the [az devops extension show](/cli/azure/devops/extension#az-devops-extension-show) command to list the details about an extension. To get started, see [Get started with Azure DevOps CLI](../cli/index.md).
...
- **Extension crashes or causes errors:**
  - **Collect diagnostic information:** If an extension crashes or causes errors, collect diagnostic information to help identify the problem. This information includes error messages, logs, and screenshots of the problem.
  - **Check extension logs:** Some extensions provide logs that can help diagnose problems. Check the extension's documentation for information on how to access and interpret these logs.
  - **Contact support:** If you can't resolve the problem, contact the extension's support team with the collected diagnostic information. Provide as much detail as possible to help them troubleshoot the problem.

- **Extension not visible after installation:**
  - **Refresh the page:** Sometimes, the extension doesn't appear immediately after installation.
  - **Check permissions:** Ensure you have the necessary permissions to view and use the extension. Some extensions require specific permissions to be visible.
...
> Select the identity for the user in the organization that you want.
 
### Q: Why can't I install an extension?

A: You can't install extensions for one of the following reasons.  
- You must be a member of the [**Project Collection Administrators** group](../organizations/security/look-up-project-collection-administrators.md) or be the [**Organization owner**](../organizations/security/look-up-organization-owner.md). If you don't have permissions, you can [request extensions](request-extensions.md) instead.
...
- Most extensions require at least **Basic** access, not Stakeholder. For more information, see the extension's description in the [Visual Studio Marketplace](https://marketplace.visualstudio.com/azuredevops), Azure DevOps tab.
- A free extension trial might have expired. To check:
   1. On your organization toolbar, select **Users**.
   1. Go to the extension pane.
   1. Check for an expiration message.

- The extension might not be assigned to the user. To check:
   1. On your organization toolbar, select **Users**.
   1. Go to the extension pane.
   1. Check whether the extension is assigned. If it isn't, assign it.

### Q: What happened to my preview extension?

A: Your extension might be out of preview. After a preview period, an extension is no longer free and becomes a paid extension. A grace period might be offered before you have to buy it. All preview extensions automatically convert to a [free trial](../organizations/billing/try-additional-features-vs.md) before purchase is required.
...
1. Sign in to your Azure DevOps Server home page (`https://{server}:8080/tfs/`) with the identity that's a user in the project collection where you want to install the extension.
   Azure DevOps might ask you to choose between a "work or school organization" or "personal account." If so, the identities share the same email address but have different profiles, security settings, and permissions. Select the identity that's the user in your project collection.
...
A: You must be a member of the [Team Foundation Administrators group](/azure/devops/server/admin/add-administrator#add-a-user-to-the-server-administrators-group) and have [**Edit instance-level information** permissions](../organizations/security/permissions.md#server) for the Azure DevOps Server where you want to upload extensions.
::: moniker-end
