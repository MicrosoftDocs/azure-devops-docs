...
With the personal access token management API, it is far easier for users to manage the PATs associated with their organizations via automated processes. This rich set of APIs enable you to better manage the PATs you own, offering you functionality such as creating new personal access tokens with a desired scope and duration, renewing existing personal access tokens, or expiring existing personal access tokens.
