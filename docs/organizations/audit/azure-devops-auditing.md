---
title: Access Azure DevOps Audit Logs, Export, and Filter
...
Azure DevOps auditing logs organization-level activities to help you maintain security and compliance. This article explains how to enable auditing, access and filter the audit log, and export audit events.

[!INCLUDE [important-backed-by-azure-active-directory](includes/important-backed-by-azure-active-directory.md)]

An audit event is recorded whenever a user or service identity within the organization changes the state of an artifact. Examples of logged events include:
...
For a full list of events, see [Auditing events](auditing-events.md).

Audit events are stored for 90 days, after which they're deleted. To retain audit data longer, back up events to an external location or use [audit streaming](auditing-streaming.md).

Auditing isn't available for on-premises deployments of Azure DevOps Server. However, you can connect an audit stream from an Azure DevOps Services instance to an on-premises or cloud-based instance of Splunk. Ensure you allow IP ranges for inbound connections. For details, see [Allowed address lists and network connections, IP addresses and range restrictions](../security/allow-list-ip-url.md#range-restrictions).
...
1. Under the **Security** header, select **Policies**.   
1. Turn the **Log Audit Events** button **ON**.

   ![Screenshot of Auditing policy enabled.](media/azure-devops-auditing/enable-auditing-policy.png)  

   This action enables auditing for the organization. Refresh the page, and then **Auditing** appears in the sidebar. Audit events start appearing in **Auditing Logs** and through any configured audit streams.
1. If you no longer want to receive auditing events, turn the **Enable Auditing** button **OFF**. This action removes the **Auditing** page from the sidebar and makes the **Auditing Logs** page unavailable. Any audit streams stop receiving events.
...
1. If **Auditing** doesn't appear in **Organization settings**, you don't have access to audit events. The Project Collection Administrators group can give permissions to other users and groups so they can access the auditing pages. To do so, select **Permissions**, and then find the group or users to provide auditing access to.
...
1. If **Auditing** doesn't appear in **Organization settings**, you don't have access to audit events. The Project Collection Administrators group can give permissions to other users and groups so they can access the auditing pages. Select **Security**, and then find the group or users to provide auditing access.
...
The **Auditing** page provides a simple view into the audit events recorded for your organization. The following table describes the information available on the **Auditing** page:
...
Some audit events, known as "bulk audit events," include multiple actions that happen at the same time. You can recognize these events by the **Information icon** on the far right of the event. To see the details for each action in bulk audit events, check the downloaded audit data.
...
As you review the audit events, use the *Category* and *Area* columns to filter and find specific types of events. The following tables list the categories and areas, along with their descriptions:

#### List of events

The product team works to add new auditing events each month. If you want an event tracked that isn't currently available, share your suggestion with them in the [Developer Community](https://developercommunity.visualstudio.com/search?space=21).

For a comprehensive list of all events that the Auditing feature can emit, see the [Auditing Events List](/azure/devops/organizations/audit/auditing-events).
...
1. To narrow the displayed audit events, select the time filter.
...
> [!NOTE]
> For long-term storage and analysis of your auditing events, consider using the [Audit Streaming feature](auditing-streaming.md) to send your events to a Security Information and Event Management (SIEM) tool. Export the auditing logs for cursory data analysis.
...
The system can audit only the following limitations:

* **Microsoft Entra group membership changes**: Audit Logs include updates to Azure DevOps groups and group membership when an event `Area` is `Groups`. However, if you manage membership through Microsoft Entra groups, these logs don't include additions and removals of users from those Microsoft Entra groups. Review the Microsoft Entra audit logs to see when a user or group was added or removed from a Microsoft Entra group.
...
A: The `DirectoryServiceAddMember` group helps manage membership in your organization. Many system, user, and administrative actions can affect membership in this system group. Since this group is used only for internal processes, disregard audit log entries that capture membership changes to this group.
