...
Your hosted agents run within the same geography as your organization. A geography is a data residency boundary that contains one or more Azure regions. While your agent might run in the same region as your organization, it isn't guaranteed to do so&mdash;it can run in any region within your geography. For example, an organization in the **Europe** geography can be assigned an agent from any region in that boundary, such as **France Central** or **Spain Central**. To obtain the complete list of possible IP ranges for your agent, you must use the IP ranges from *all* of the regions in your geography, not just your organization's region.
...
Microsoft-hosted agents are provisioned within your organization's geography, which is a data residency boundary. When you allowlist agent IP ranges, you must include the ranges for *every* region in your geography, because your agent can run in any of them. The following table lists the regions in each geography.

> [!NOTE]
> These geographies are specific to Microsoft-hosted agents and align with data residency boundaries. They don't map one-to-one to the geographies shown on the [Azure geographies](https://azure.microsoft.com/global-infrastructure/geographies/) page. For example, the **Europe** geography for Microsoft-hosted agents spans regions that the Azure geographies page lists under separate France, Sweden, Poland, Spain, Austria, and Belgium geographies. Organizations in the European Union are always assigned agents from regions within the EU data residency boundary.
