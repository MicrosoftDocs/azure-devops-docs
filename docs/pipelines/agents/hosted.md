...
Microsoft-hosted agents are provisioned from the regions mapped to your organization's Azure geography. This mapping differs from the standard Azure geography-to-regions mapping, but it still follows the same data residency restrictions. Because your agent can be assigned from any of the mapped regions, you must allowlist the IP ranges for *every* region in your geography. The following table lists the regions mapped to each geography for Microsoft-hosted agents.
