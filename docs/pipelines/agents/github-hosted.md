...
This naming model enables developers to target the desired Apple toolchain. It reflects how macOS CI jobs are commonly defined, which is by their required Xcode toolchain rather than by the underlying operating system. The operating system might change over the lifetime of the image. To see the software included with the Xcode 27 image, including the macOS version, follow the included software link for the image in the previous table.
