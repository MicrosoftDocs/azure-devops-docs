...
Configure continuous integration (CI) and continuous delivery (CD) for your **Go** application with The **Azure DevOps Project**.  The Azure DevOps project simplifies the initial configuration of a VSTS build and release pipeline.
...
1. Create a **new** free VSTS account or choose an **existing** account.  Choose a **name** for your VSTS project.  Select your **Azure subscription**, **location**, and choose a **name** for your application.  When you're done, choose **Done**.
    ...
    ![Enter VSTS info](_img/azure-devops-project-go/vstsazureinfo.png)
...
1. Select the **ellipsis**.  This action opens a menu where you can perform several activities such as queue a new build, pause a build, and edit the build pipeline.
