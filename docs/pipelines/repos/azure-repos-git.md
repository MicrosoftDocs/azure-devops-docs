...
> [!NOTE]
> If you push a new tag to a commit that has `[skip ci]` in its message, the pipeline runs on that commit if the tag matches the tag filters for that branch's CI trigger. The tag's message isn't evaluated, so the pipeline runs even if the tag has a `[skip ci]` message.
