...
> [!NOTE]
> A given task or job can't unlaterally decide whether the job/stage continues. What it can do is offer a status of **succeeded** or **failed**, and downstream tasks/jobs each have a condition computation that lets them decide whether to run or not. The default condition is effectively "run if we're in a successful state".
> 
