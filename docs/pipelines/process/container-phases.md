...
Starting with [Agent version 5.279.0](https://github.com/microsoft/azure-pipelines-agent/releases#release-v5.279.0), container jobs on Linux agents don't mount the host Docker socket (`/var/run/docker.sock`) into the job container by default. This default follows the principle of least privilege, so jobs that don't need Docker-in-container behavior aren't granted access to the host Docker daemon.
