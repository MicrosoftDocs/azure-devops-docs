...
Similar to an [agent pool](../../agents/pools-queues.md), a deployment group is a logical set of target machines. Each group has a deployment agent installed.

Deployment groups typically represent environments such as *development*, *test*, or *production*. Each physical machine or virtual machine (VM) in the deployment group communicates with Azure Pipelines to coordinate deployment tasks. Deployment groups differ from [deployment jobs](../../process/deployment-jobs.md), which are collections of task-related steps defined in YAML pipelines.
...
Deployment groups are available only in classic release pipelines.
...
:::image type="content" source="media/generated-script.png" alt-text="Screenshot that shows how to create a deployment group in Azure Pipelines.":::

## Register target servers

To register each target server in the deployment group:

1. Sign in to the machine by using an account that has administrator permissions, and run the copied script. For Windows machines, use an elevated PowerShell command prompt.

1. When you're prompted to add tags, enter **Y**, and then provide one or more tags.

   You can use tags to target deployments to specific servers in a [deployment group job](../../process/deployment-group-phases.md). Tags are case insensitive and limited to 256 characters each. There's no limit to the number of tags that you can use.

1. After setup is complete, the script should return the following message:
...
To upgrade deployment agents to the latest version without redeploying, go to the **Deployment groups** pane, select the **More actions** ellipsis next to your deployment group, and then select **Update targets**. For more information, see [Azure Pipelines agents](../../agents/agents.md).

:::image type="content" source="media/update-targets-deployment-groups.png" alt-text="Screenshot that shows how to update targets in deployment groups.":::

## Create and manage deployment pools
...
To upgrade the agent version on all servers in a deployment pool, go to **Deployment pools**, select the **More actions** ellipsis next to the pool, and then select **Update targets**.

:::image type="content" source="media/update-targets-pools.png" alt-text="Screenshot that shows how to update targets in deployment pools.":::
...
1. Select **Pipelines** > **Deployment groups**. Select your deployment group, and then select **Manage**.
   ...
   :::image type="content" source="media/manage-deployment-group.png" alt-text="Screenshot that shows the details of a deployment group.":::

1. In the list of projects, select the ones that you want to share the deployment group with, and then select **Save**.

   :::image type="content" source="media/deployment-group-share-with-project.png" alt-text="Screenshot that shows how to share with other projects.":::

1. Confirm that for the selected projects, the shared deployment group is listed on the **Deployment groups** pane.

   :::image type="content" source="media/shared-deployment-group.png" alt-text="Screenshot that shows a shared new deployment group.":::
...
1. Select the post-deployment icon to open the **Post-deployment conditions** pane, and then enable **Auto redeploy trigger**.
...
:::image type="content" source="../media/deployment-group-add-targets.png" alt-text="Screenshot that shows how to enable the trigger for automatic redeployment.":::

## Related content

- [Classic release triggers](../triggers.md)
- [Manage security in Azure Pipelines](../../policies/permissions.md)
- [Deploy web apps to Azure VMs by using deployment groups](deploying-azure-vms-deployment-groups.md)
