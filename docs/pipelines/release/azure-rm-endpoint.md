...
1. In Azure DevOps, open the Service connections page from the [project settings page](/azure/devops/project/navigation/go-to-service-page?view=azure-devops&preserve-view=true#open-project-settings). In TFS, open the **Services** page from the "settings" icon in the top menu bar.
...
> [!NOTE]
> Managed identities are not supported in Microsoft Hosted Agents. You will have to [set-up a self hosted agent](/azure/devops/pipelines/agents/agents?view=azure-devops&preserve-view=true#install) on an Azure VM and configure managed identity for the virtual machine.
