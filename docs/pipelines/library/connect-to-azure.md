...
* [App registration or managed identity (manual) with workload identity federation or a secret](../release/configure-workload-identity.md#managed-identitytabmanaged-identity). Manual configuration is more time consuming than the automatic configuration and should only be used if you've already tried to automatic option. 
