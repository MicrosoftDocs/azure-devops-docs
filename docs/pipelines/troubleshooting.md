...
1. Queue the build or release.

2. In the explorer tab, view your completed build or release and click the failing task to view its output.

3. If you need a copy of all the logs, click **Download all logs as zip**.
...
2. On the build summary page, there will now be a **Diagnostic logs** section. You can download your diagnostic logs per job. If you would like to download everything you can also choose to **Download all logs as zip**.
...
1. We recommend you listen only to agent traffic.  File > Capture Traffic off (F12)  

2. Enable decrypting HTTPS traffic.  Tools > Fiddler Options > HTTPS tab. Decrypt HTTPS traffic

3. Let the agent know to use the proxy:

   ```cmd
   set VSTS_HTTP_PROXY=http://127.0.0.1:8888
   ```

4. Run the agent interactively.  If you're running as a service, you can set as the environment variable in control panel for the account the service is running as.
...
1. Charles: Proxy > Proxy Settings > SSL Tab.  Enable.  Add URL.  

2. Charles: Proxy > Mac OSX Proxy.  Recommend disabling to only see agent traffic.

   ```bash
   export VSTS_HTTP_PROXY=http://127.0.0.1:8888
   ```

3. Run the agent interactively.  If it's running as a service, you can set in the .env file.  See [nix service](https://github.com/Microsoft/azure-pipelines-agent/blob/master/docs/start/nixsvc.md)
