...
This section will aim to provide few troubleshooting tips for some of the common issues that a user may face when using the MSBuild task.

### Failure

Build failed with the following error: <code>An internal failure occurred while running MSBuild</code>
...
##### Running the pipeline with diagnostics to retrieve the different types of logs
...
You can also setup and download a customized verbose logs to assist with your troubleshooting:
...
In addition to the pipeline diagnostic logs, you can also check the other types of logs below that contain more information that can help you to debug and solve the problem:
...
##### Trying to reproduce the error locally

If you are using a hosted build agent, you may want to try to reproduce the error locally. This will help you to narrow down weather the failure is the result of the build agent or the build task.
...
At the bottom of this page, check out the GitHub issues in the <code>Open</code> and <code>Closed</code> tabs to see if there is a similar issue that have been resolved previously by our team.

We also welcome your questions, suggestions and feedback on [Developer Community](https://developercommunity.visualstudio.com/spaces/8/index.html)