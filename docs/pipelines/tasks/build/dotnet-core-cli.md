...
<tr><td><code>publishWebProjects</code><br/>Publish Web Projects</td><td>If true, the task will only try to find the web projects in the repository and run the publish command on them. Web projects are identified by presence of either a web.config file or wwwroot folder in the directory.</td></tr>
