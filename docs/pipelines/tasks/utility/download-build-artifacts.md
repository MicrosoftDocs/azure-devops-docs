...
|`extractTars`<br/>Extract all files that are stored inside tar archives|(Optional) Extract all .tar files. Enabling `StoreAsTar` option in [PublishBuildArtifacts](./publish-build-artifacts.md) task will store artifacts as .tar files automatically. This option allows you to preserve Unix file permissions. Not available on Windows. <br/>Default value: `false`|
