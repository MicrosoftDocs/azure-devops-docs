...
* A JSON object may contain an array whose values can be referenced by their index.
  ...
  use the variable name `FeatureFlags.Preview.1.NewWelcomeMessage`. However, the file transform task (https://docs.microsoft.com/en-us/azure/devops/pipelines/tasks/utility/file-transform) has the ability to transform entire arrays in JSON files. And you can directly use DBAccess.Users = ["NewUser1","NewUser2","NewUser3"]
