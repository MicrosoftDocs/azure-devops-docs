...
1.	[Helm Tool installer task](../tool/helm-installer.md) can be used to get the right version of Helm on the agents.
2.	Helm package and deploy task can be used to package the application and deploy it to a Kubernetes cluster. 
     You can use the task to install/update Tiller to a Kubernetes namespace, securely connect to the Tiller over TLS for deploying charts, or to run any Helm command like lint
3.	The Helm task also supports connecting to an Azure Kubernetes Service by using Azure Service Connection. You can connect to any Kubernetes cluster by using kubeconfig or Service Account as well.
4.	The Helm deployments can be supplemented by using Kubectl task. For example create/update imagepullsecret
