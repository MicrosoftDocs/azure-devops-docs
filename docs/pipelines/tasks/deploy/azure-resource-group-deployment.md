...
<tr><td>TFS/VSTS endpoint</td><td>(Required) Agent registration with Deployment group requires access to your Visual Studio project.​ <br><br>Click "Add" to create an endpoint using personal access token (PAT) with scope restricted to "Deployment Group" and a default expiration time of 90 days. <br><br>​Click "Manage" to update endpoint details.​</td></tr>
<tr><td>Team project</td><td>(Required) Specify the Team Project which has the Deployment Group defined in it​</td></tr>
