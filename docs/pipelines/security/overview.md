...
- **Add additional security for forks**: When you work with public repositories from GitHub, carefully consider your approach to fork builds. Forks originating from outside your organization pose particular risks.    
    ...
    - **Review PR comment requirements for both PR sources**: Pull request comment requirements for triggering validation runs can be configured independently based on whether the pull request originates from the same repository or from a fork. Review the settings for both sources to make sure the intended security posture is applied to external (fork) contributions as well as internal pull requests.
...
- **Use the Azure DevOps service connection for PAT-free access to Azure DevOps resources**: Azure DevOps Services supports an [Azure DevOps service connection](../library/add-devops-entra-service-connection.md) (preview) that authenticates using Microsoft Entra workload identities (a service principal or managed identity) instead of personal access tokens (PATs) or session tokens. Consider it as a more secure alternative when pipelines need to access Azure DevOps resources, such as cross-organization repository checkout, YAML template reuse, or Azure Artifacts feeds.
