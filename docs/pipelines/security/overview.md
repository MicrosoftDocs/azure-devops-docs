...
- **Add additional security for forks**: When you work with public repositories from GitHub, carefully consider your approach to fork builds. Forks originating from outside your organization pose particular risks.    
    ...
    - **Review PR comment requirements for both PR sources**: Pull request comment requirements for triggering validation runs can be configured independently based on whether the pull request originates from the same repository or from a fork. Review the settings for both sources to ensure the intended security posture is applied to external (fork) contributions as well as internal pull requests.
