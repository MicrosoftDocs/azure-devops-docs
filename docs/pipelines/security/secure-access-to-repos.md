...
The GitHub Azure Pipelines app is the recommended authentication type for continuous integration pipelines. Builds and GitHub status updates use the Azure Pipelines identity instead of your personal GitHub identity. OAuth and PAT authentication use your personal GitHub identity and must be authorized for pipeline access. For more information, see [Access to GitHub repositories](pipeline/repos/github.md#access-to-github-repositories).
...
- Use Microsoft-hosted agents to build from forks. Resources are immediately deleted from the agents after builds. If you use self-hosted agents, clean and update the agents regularly, or use separate agents for different repositories or branches.

## Other repository security measures

- [Use YAML pipelines instead of Classic pipelines](pipelines/security/approach.md#use-yaml-pipelines-instead-of-classic-pipelines) to reduce security risks from YAML and Classic pipelines sharing resources. Disable creation of Classic pipelines by enabling **Disable creation of classic build pipelines** and **Disable creation of classic release pipelines** toggles in **Project Settings** or **Organization Settings**. These toggles are enabled by default for new organizations.
