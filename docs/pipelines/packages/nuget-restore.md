...
- [Created a build](/vsts/pipelines/) for that solution
...
If your NuGet.config contains feeds in a different VSTS organization (*organization*.visualstudio.com) than the organization running the build, you'll need to set up credentials for those feeds manually.

1. Select a login account (either a service account (recommended) or a user's account) that has access to the remote feed
2. Using your browser's InPrivate mode, Incognito mode, or similar, go to the VSTS organization that contains the feed, sign in with the login account you selected in step 1, click the user profile circle in the top right, and select Security
3. Create a PAT with the **Packaging (read)** scope and keep it handy
4. In the VSTS organization that contains the build, edit the build's NuGet step and ensure you're using version 2 or greater of the task, using the version selector
