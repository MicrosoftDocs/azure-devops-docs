...
The next example publishes to a custom registry defined in your repo's `.npmrc` file. You'll need to set up an [npm service connection](/vsts/pipelines/library/service-endpoints?view=azure-devops#sep-npm&preserve-view=true) to inject authentication credentials into the connection as the build runs.
