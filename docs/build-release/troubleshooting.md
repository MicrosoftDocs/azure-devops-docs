...
By default the build steps such as [MSBuild](steps/build-release/msbuild.md) and [Visual Studio Build](steps/build-release/visual-studio-build.md) run MSBuild with the `/m` switch. In some cases this can cause problems such as multiple process file access issues. 
