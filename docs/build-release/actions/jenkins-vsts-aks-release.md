...
# Tutorial:  Deploy to Azure Container Service (AKS) with Jenkins CI and VSTS CD

Visual Studio Team Services (VSTS) provides integration with Jenkins so that you can:

* Continue to use your existing investment in Jenkins.
* Create a single release process using VSTS that deploys to a variety of Azure targets.

In this tutorial, you use Jenkins for Continuous Integration (CI) and VSTS for Continuous Delivery (CD) to deploy a **Spring Boot app** to an **Azure Container Service (AKS) cluster**.  

You will:

> [!div class="checklist"]
> * Get the Sample App
> * Use Jenkins to build the Docker image for the Spring Boot app using Maven and push the image to Azure Container Registry (ACR)
> * Use VSTS to deploy to an AKS cluster by pulling the Docker image from the ACR.
...
* A Spring Boot app.  You can fork the sample app found [here](https://github.com/spring-guides/gs-spring-boot-docker.git).

* An Azure Container Registry (ACR).  You can follow  steps to deploy an ACR and login to the registry using the Azure CLI found [here](https://docs.microsoft.com/en-us/azure/aks/tutorial-kubernetes-prepare-acr#deploy-azure-container-registry).
...
* Access to a Jenkins server with Maven configured. If you have not yet created a Jenkins server,
...
1. Filter the **available list** to find the **VS Team Services Continuous Deployment** plugin and choose the **Install without restart** option.  You can find additional information about the **VSTS plugin** 
...
Before you create the release definition, you need to install an extension that will be used to replace the **K8sDeploy.yaml** file's **__Build.BuildId__** token with the current Jenkins build id. 

1. Select the **Browse Marketplace** menu item.

2. Search for the **Release Management Utility Tasks** extension provided by **Microsoft DevLabs**. 
...
Once the app is deployed to the AKS cluster, query the external IP address using  **kubectl**, the Kubernetes command-line client.  You can learn how to install and connect **kubectl** to your AKS Cluster by following [these steps](https://docs.microsoft.com/en-us/azure/aks/tutorial-kubernetes-deploy-cluster#install-the-kubectl-cli).

Here is the command for querying the external IP address for the deployed app:
...
You can then use the external IP address to open the sample app in a web browser.
...
> [!div class="checklist"]
> * Get the sample app.
> * Configure Jenkins CI job that does the following:
>     * Builds a Docker image that is tagged with the current Jenkins build id.
>     * Authenticates and pushes the Docker image to a private registry (ACR).
>     * Archives the Kubernetes YAML configuration file that is needed for deployment.
> * Configure a VSTS Release Definition for CD to Azure that does the following:
>     * Updates the YAML configuration file to deploy an image tagged with the current Jenkins build id.
>     * Configures the AKS cluster to deploy using the YAML configuration file.
>     * Stores the authentication token used to pull images from the ACR in a secret on the AKS cluster.
> * Test the CI/CD pipeline with a pull request.
> * Browse to the deployed sample app in the browser using the exposed external IP.
