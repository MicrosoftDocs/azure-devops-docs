...
> [!NOTE]
> Identities used by tools and applications to integrate with Azure DevOps can occasionally need higher rate and usage limits beyond the allowed consumption limit. To increase these limits, assign the [Basic + Test Plans](../../organizations/billing/buy-basic-access-add-users.md#assign-basic-or-basic--test-plans) access level to the identity your application uses.
> 
> After you no longer need higher rate limits, revert to the previous access level. You're charged for Basic + Test Plans only for the duration it's assigned. Only the Basic + Test Plans access level provides an increase to these limits.
> 
> If the identity is already assigned the Visual Studio Enterprise access level, remove that subscription before you assign Basic + Test Plans.
