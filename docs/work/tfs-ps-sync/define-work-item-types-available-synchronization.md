...
-   Adds the **Project Server** tab to the forms for the types of work items that you specify.  
 ...
 If you try to map work item types for a team project that is not associated with an enterprise project plan, an error appears. You can map work item types only for team projects that are configured to participate in synchronization. For more information, see [Associate enterprise projects and team projects](manage-associations-enterprise-projects.md).  
...
### List work item types that are mapped  
 The following example retrieves the types of work items that are configured to participate in synchronization for the Contoso team project in DefaultCollection on AdventureWorksServer.  
