...
|Data fields that are synchronized|[Customize the field mapping](customize-field-mapping-tfs-project-server.md)<br /><br /> To synchronize data between an enterprise project plan and a team project, you must associate the work-item fields in Team Foundation with the fields in Project Server. You can add fields and specify how they synchronize. For example, you can share data that does not relate to schedules (such as cost centers, team names, or health status) if you add the fields that store these types of data to the mapping file.<br /><br /> You can add fields or just use the default set of fields that are required to support synchronization.|  
