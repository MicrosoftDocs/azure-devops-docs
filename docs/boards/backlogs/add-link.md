...
1. In the **Link type** dropdown list, select the type of link you want to create (for example, Parent, Child, Related, and so on).
...
You can't construct a query that shows a hierarchical view of Test Plans, Test Suites, and Test Cases. These items aren't linked together using Parent/Child or any other link type. You can only view the hierarchy through the **Test** > **Test Plans** page. For more information, see (../../test/create-a-test-plan.md).
