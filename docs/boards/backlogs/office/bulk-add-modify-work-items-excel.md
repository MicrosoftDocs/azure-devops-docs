...
author: chcomley
ms.custom: linked-from-support, copilot-scenario-highlight
ms.topic: tutorial
ai-usage: ai-assisted
monikerRange: '<= azure-devops'
ms.date: 07/16/2026
#customer intent: As a project member, I want to add multiple work items at once with Excel, so that I can save time setting up my project backlog.
---
...
> - Use the [web portal](../bulk-modify-work-items.md) for interactive bulk edits.
> - Use [CSV import/export](../../queries/import-work-items-from-csv.md) for bulk import and update scenarios.

## Choose your scenario

- [Add new work items as a flat list](#import-work-items-as-a-flat-list)
- [Add hierarchical work items as a tree list](#import-work-items-as-a-tree-list)
- [Update existing work items from a query](#update-work-items-in-bulk-with-a-query-list)
- [Manage links and attachments](#link-work-items)
...
For more information, see [Client compatibility with Azure DevOps Server](../../../organizations/projects/connect-to-projects.md#requirements-and-client-compatibility).
...
- **Query list, flat list**: Create an Excel report based on the query of work items. To create an Excel report, your project collection must be configured to support Analytics reporting. For more information, see [Create a report](#create-a-report).
...
To stay in sync with Azure Boards, publish and refresh frequently. For more information about Excel features, see [Basic Excel tasks](https://support.office.com/article/basic-tasks-in-excel-dc775dd1-fa52-430f-9c3c-d998d1735fca).

### Sort work items

Use the following guidance when sorting work items in Excel:

- Sort **flat lists** by using the [Excel sort feature](https://support.office.com/article/sort-data-in-a-range-or-table-62d0b95d-2a90-4610-a6ae-2e545c4a4654).
...
- **Reorder items in a team backlog**: Use the [Stack Rank](../../queries/planning-ranking-priorities.md#fields-table) or [Backlog Priority](../../queries/planning-ranking-priorities.md#fields-table) field (Agile or Scrum process). Set values, publish the worksheet, and refresh the backlog. Items appear from lowest to highest number. If someone reorders the backlog later, these values can change.
...
- **Bulk tag updates**: Add the **Tags** field, and then assign one or more tags separated by semicolons (`;`). For tagging guidance, see [Categorize and filter lists and boards with work item tags](../../queries/add-tags-to-work-items.md).
- **Plain-text updates to rich-text fields**: Enter simple text in rich-text fields. When you update multiple work items in bulk, some existing rich formatting might be lost.
- **Offline edits**: Work offline, then reconnect and publish changes. For details, see [Connect Azure Boards to an Office client, Work offline, and reconnect](track-work.md).
...
1. In Excel, start with a blank worksheet. If the **Team** ribbon isn't available, ensure that you installed the [Azure DevOps Office Integration 2019 tool](https://visualstudio.microsoft.com/downloads/#other-family). For more information, see [Azure DevOps Office integration issues](tfs-office-integration-issues.md). 
...
1. Clear all values under the **Title \<Number>** column you want to remove.
...
To troubleshoot invalid links, see [FAQs](./faqs.yml#how-do-i-resolve-invalid-links-in-a-tree-list).
...
>
> - Start with :::image type="icon" source="media/bulk-modify-excel-refresh-inline.png"::: **Refresh** to pull the latest data.
> - Use :::image type="icon" source="media/bulk-modify-excel-choose-columns-inline.png"::: **Choose Columns** to add fields you need to edit.
> - Publish frequently to reduce data conflicts.
> - Save the workbook regularly to avoid losing unpublished changes.

1. Create a query that returns the work items you want to update. For more information, see [Create and save managed queries with the query editor](../../queries/using-queries.md).

1. Open Excel and connect to your Azure Boards project. Use one of the four methods provided in [Connect Azure DevOps project to Excel](track-work.md).

1. Continue with the path that matches where you started:

   - **Path A: You opened the query from the web portal or Visual Studio**
   
      1. Select [**Choose Columns**](#add-or-remove-column-fields) in the **Team** ribbon to add any fields you want to edit.
      
      1. Update values, and then publish your changes.
   ...
   - **Path B: You started in Excel**
      ...
      1. In the **New List** dialog, select **Query list**, and then select the query:
         ...
         The icon next to each query shows the query type. For details, see [Query types](#query-types).
      ...
      1. Make your updates, and then publish changes.
...
For step-by-step instructions, see [Change your list type or query](#change-your-list-type-or-query).
...
[Modify your query](../../queries/using-queries.md) to include the work items you want, and then refresh the worksheet.
...
1. Select **Find**.
   ...
   - The :::image type="icon" source="../../../boards/media/icons/checkmark.png"::: checkmark indicates a work item already in the worksheet.
   - To sort results, select a column header. You can also resize columns to see more details.

1. In the results list, select the :::image type="icon" source="../../../boards/media/icons/checkmark.png"::: checkbox for each work item you want to add.
...
1. Confirm that Visual Studio 2015.1 or later is installed. If needed, [download and install a newer version](https://visualstudio.microsoft.com/downloads/).

1. In your worksheet, select an identity or person-named field. This selection enables **Select User** in the **Team** ribbon:
...
1. In the **Assign User** dialog, start typing a user name and select the matching account from the filtered results.
...
For more information about linking work items, see [Link user stories, issues, bugs, and other work items](../add-link.md).
...
#### Link one work item to other work items
...
1. In the **Add link to \<Work Item>** dialog, identify the work items you want to link:

   1. Select the **Link Type**. The same link type is applied to all selected work items.
   
   1. Select the identifiers (**Work item IDs**) for all work items to link. For more information, see [Find work items to link](#find-work-items-to-link).
...
1. Select **Publish** in the **Team** ribbon, and then close the dialog.

#### Link the same target work items to multiple work items
...
1. Publish your changes. The identified work items are linked to all selected work items in the worksheet.

### Find work items to link   

In the **Add link to \<Work Item>** dialog, open **Choose Linked Work Items** to select one or more target work items. If you want to use a saved query, first [define the query](../../queries/using-queries.md).
...
1. In the **Choose Linked Work Items** dialog, select the method to get the work items to link:
   ...
   Configure this dialog the same way as [**Get Work Items**](#get-work-items-dialog). For more information, see [Add existing work items to your worksheet](#add-existing-work-items-to-your-worksheet).
...
1. In the **Column Options** dialog, select the fields you want to display:
   ...
   - To add fields, select one or more fields in **Available columns**, and then select the :::image type="icon" source="../../../boards/media/icons/right-arrow.png"::: **Add selected column** arrow.

   - To remove fields, select one or more fields in **Selected columns**, and then select the :::image type="icon" source="../../../boards/media/icons/left-arrow.png"::: **Remove selected columns** arrow.
...
1. In the **Links and Attachments** dialog, on the **Links** tab, select the link to update, and then select :::image type="icon" source="media/icons/edit.png"::: **Edit Link**.
...
1. To change the linked work item, enter the work item ID, or select **Browse** to locate it.

   Configure fields in this dialog the same way as [**Get Work Items**](#get-work-items-dialog). For more information, see [Add existing work items to your worksheet](#add-existing-work-items-to-your-worksheet).
...
1. Select **Publish**, and then close the dialog.
...
For flat-list queries, you can create charts and reports directly in the web portal. For more information, see [Track progress by creating status and trend query-based charts](../../../report/dashboards/charts.md).
...
For additional details, see [Create a report](#create-a-report).

## Resolve publishing errors  

Use the following references when publish or refresh operations fail in Excel:

- [Resolve data conflicts](tfs-office-integration-issues.md#resolve-data-conflicts-when-you-publish-or-refresh-excel-data): Another update changed the field value in Azure Boards after your last publish.

- [Resolve data validation errors](tfs-office-integration-issues.md#resolve-data-validation-errors-that-occur-when-you-publish-from-excel): A field value violates the rules for the work item type.

- [Resolve invalid links in a tree hierarchy](faqs.yml#how-do-i-resolve-invalid-links-in-a-tree-list): Tree links were broken by a move or sort operation. Review the error and reposition items to restore the hierarchy.

- [Address Error TF208104: Hierarchical Link Relationship Is Locked](faqs.yml#how-do-i-resolve-invalid-links-in-a-tree-list): Field changes publish, but hierarchy changes don't, because a process such as Project Server integration locks one or more link relationships.

## Troubleshoot common issues

Use this section as a quick recovery guide while you work in Excel.

- **Team ribbon isn't available**: Verify that Azure DevOps Office Integration is installed. For more information, see [Azure DevOps Office integration issues](tfs-office-integration-issues.md).
- **Publish fails with validation errors**: See [Resolve data validation errors](tfs-office-integration-issues.md#resolve-data-validation-errors-that-occur-when-you-publish-from-excel).
- **Publish fails with data conflicts**: Refresh the worksheet, reapply changes, and publish again. For more information, see [Resolve data conflicts](tfs-office-integration-issues.md#resolve-data-conflicts-when-you-publish-or-refresh-excel-data).
- **Tree links are invalid after edits**: See [Resolve invalid links in a tree hierarchy](faqs.yml#how-do-i-resolve-invalid-links-in-a-tree-list).
- **TF208104 hierarchical link is locked**: See [How do I resolve invalid links in a tree list](faqs.yml#how-do-i-resolve-invalid-links-in-a-tree-list).

<a id="use-ai-assistance"></a>

## Use AI to add or modify work items without Excel

If you configure the [Azure DevOps MCP Server](../../../mcp-server/mcp-server-overview.md), you can describe bulk add and modify tasks in natural language instead of publishing and refreshing an Excel worksheet.
...
> [!NOTE]
> If you're using Visual Studio Code, [agent mode](/visualstudio/ide/copilot-chat-context#agent-mode) is especially helpful for complex bulk work item operations.
