...
| Task | Example prompt |
|------|----------------|
| View team work items | ```copilot-prompt\nList all active features assigned to the Platform team's area path\n``` |
| Create items at portfolio level | ```copilot-prompt\nCreate a new epic called 'Q3 Infrastructure Upgrades' and assign it to the management team\n``` |
| Check cross-team progress | ```copilot-prompt\nShow me all in-progress user stories across all feature teams under the <Contoso> area path\n``` |
| Assign work to a feature team | ```copilot-prompt\nUpdate user stories #101, #102, and #103 to the area path <Contoso>\\Mobile\n``` |
