...
> [!NOTE]   
> Use [Delivery Plans](../plans/track-dependencies.md) to track dependencies instead of Dependency Tracker. The Dependency Tracker extension isn't a supported feature of Azure Boards and no product team supports it. For questions, suggestions, or problems you have when using the extension, visit the [Marketplace for Azure DevOps, Dependency Tracker extension](https://marketplace.visualstudio.com/items?itemName=ms-eswm.dependencytracker&ssr=false#overview) page. The Dependency Tracker extension is only available on Azure DevOps Services. 

The Dependency Tracker extension helps you manage dependencies across teams, projects, and organizations. It provides filterable views to show all dependencies your team is consuming and producing. These views allow you to track the state and schedule of dependencies so you can assess the risk of dependencies to product deliverables. 
...
From the Dependency Tracker, you can choose different views and filters, and drill down to obtain specific details. The following sections describe these views and options: 
...
- Dependencies your team has on other teams for work your team is delivering.
- Dependencies other teams have on work your team is delivering. 

All teams across organizations can participate in tracking dependencies. 

> [!NOTE] 
> Dependency Tracker doesn't replace the in-person interactions that are required to agree to do the work. It provides easier planning and tracking capabilities. All parties should agree on dependencies before adding them to Dependency Tracker.
...
- **Producer**: The team that's doing the work.
  - Once the work gets submitted to the producer, the producer owns the work item.
    ...
    - The consumer shouldn't change these values once the work item gets handed off.
...
> [!IMPORTANT]  
> The default configuration for Dependency Tracker supports the Agile process. If your projects are based on a different process or you customized your process, you might need to modify the configuration. See [Configure the Dependency Tracker](#configuration) later in this article.
...
To show the filter toolbar, select the :::image type="icon" source="../../media/icons/filter-icon.png" border="false"::: filter icon. 

> [!div class="mx-imgBorder"]  
> ![Screenshot showing Dependency Tracker Filter options.](media/tracker/filter.png)  

Toggle filters on and off by selecting the filter icon. To see more filters, use the arrows at the end of the list of filters. 

Select one or more values from the drop-down menu for each field. These fields populate with the following values:

- **Work item type**: Select one or more check boxes for the work item types you want to view. Work item types configured to participate in dependency tracking. The default work item types are: Epic, Feature, User Story, and Bug. To modify the configuration, see [Configuration of Dependency Tracker](#configuration).
- **State**: Select one or more check boxes for the  work item states you want to view. The drop-down list includes all workflow states defined for all work item types shown in the selected view.  
- **Iteration**: Select one or more check boxes for the Iteration Paths you want to view. The drop-down list includes all Iteration Paths [configured for the project](../../organizations/settings/set-iteration-paths-sprints.md) and for which there are work items listed in the current view.
- **Priority**:  Select one or more check boxes for the priorities you want to view. The priority values assigned to work items.  
- **Partner**: The partner organization for which the work item is defined. 

Your filtering options depend on the [configuration](#configuration) defined for the Dependency Tracker and correspond to work items shown in the selected view that meet the filter criteria. For example, if you don't have any work items assigned to Sprint 4, then the Sprint 4 option doesn't appear in the filter options for the Iteration Path. 
...
Several views provide interactive visualizations through drill-downs. The tabbed views descriptions later in this article address these features. 
...
A dependency represents work where one team depends on another team. Both teams should track their own work in their own area path. By linking the work that depends on the other team's work, you can visualize and track the dependencies.  
...
1. Select **New Dependency**.  
    ...
    If the partner team is in a different organization, select **Partner Account**. You can enable or disable the **Partner Account** option by [configuring the Dependency Tracker](#configuration).   

    > [!div class="mx-imgBorder"]  
    > ![Screenshot of Create dependency dialog, Choose partner account.](media/tracker/choose-partner-organization.png)

2. Search for work items by ID or enter a keyword contained within the work item title. In the following example, you link a user story and a bug. 
    ...
    - The fastest way to create a dependency link is to type the **Producer** and **Consumer** work item IDs in the search boxes and then select **Save**.

    Optionally, you can select **Create New** to add work items that you then link as dependent upon each other. In the following example, you create two new features and link them. 
...
3. Select **Save**. The **Save** button is available only after you choose two work items to link.      
...
4. From the success confirmation dialog, select **View dependency**. 
   ...
   The work items that you linked are highlighted.    
    ...
    In the following example, the Fabrikam Fiber/Service Delivery/Voice team depends on the MyFirstProject team to deliver their `User Story 706: Cancel order form` to complete `Bug 390: Cancel order form`. 
...
You can also link work items by using the **Links** tab to create Successor and Predecessor links. A Predecessor is the producer of the work item, or the item that must come first. A Successor is the consumer of the work item, or the item that's dependent on the first item.

> [!div class="mx-imgBorder"]  
> ![Screenshot of Add Link dialog, Create links manually.](media/tracker/create-links-manually.png)

> [!NOTE]   
> The Successor and Predecessor (consumes and produces) link types are the default link types used by the Dependency Tracker. If you customize your projects by using a Hosted XML process model, you can specify different link types in the Dependency Tracker configuration. For more information, see [Configure the Dependency Tracker](#configuration) later in this article, and [Link user stories, issues, bugs, and other work items](../backlogs/add-link.md). 
...
You can also create a custom query by selecting the **Work items and direct links** query type and choosing any work item type. 

Select **Copy to HTML** to copy the selected work items to the clipboard as a formatted table. 

<a id="consume"></a>

## Consuming dependencies view

The **Consuming Dependencies** view shows work that a team depends on by other teams or area paths.

> [!div class="mx-imgBorder"]  
> ![Screenshot of Dependency Tracker, Consuming dependencies view.](media/tracker/consuming-dependencies-view.png)

Each column on the bar chart represents another area path that produces dependencies by workflow state for the selected **Area View**. The table shows the number of unique dependencies. It also lists all work items participating in the filtered view of tracked dependencies. 
...
- Expand or collapse the list of work items to show the full list of dependent work items by using the :::image type="icon" source="../media/icons/expand_icon.png" border="false"::: expand and :::image type="icon" source="../media/icons/collapse_icon.png" border="false"::: collapse icons. 
...
The **Producing Dependencies** view shows work that other teams or area paths depend on for the selected area.

> [!div class="mx-imgBorder"]  
> ![Screenshot of Dependency Tracker, Producing dependencies view.](media/tracker/producing-dependencies-view.png)

Each column on the bar chart represents another area path that consumes dependencies by workflow state for the selected **Area View**. The table shows the number of unique dependencies and lists all work items included in the filtered view of tracked dependencies. 
...
> [!IMPORTANT]   
> For the Timeline to show meaningful data, assign the dependent work items to iteration paths, and assign start and end dates to those iteration paths.  
...
There are two views: **Consuming From** and **Producing For**. You can configure the workflow state color coding. The width of the lines indicates how many dependencies exist in that area. The thicker the link, the more dependencies, as indicated in the legend.
...
To modify the configuration, be a member of the Project Collection Administrator Group. All changes to the configuration apply to all projects defined in the organization. 

To change the configuration, select the  :::image type="icon" source="../../media/icons/blue-gear.png" border="false":::  gear icon and modify the syntax listed. Select **Save** when you're done. 
...
The `newDependencyButtonEnabled` property enables or disables the **New Dependency** link option. When enabled, the link appears on the Dependency Tracker page. When disabled, users can't create dependencies from the tracker, and they can only review the dependencies created through other means. The default value is `true` (enabled).  

### Enable or disable cross-organization linking

The `crossAccountConfigs` property enables or disables cross-organization dependency linking from the New dependency dialog. The default value is `true` (enabled).  

To disable it, set the following syntax in the JSON configuration to `false`.
...
Cross account linking requires the use of a special link type and you should only use it in coordination with the **New Dependency** option.
...
:::row:::
   ...
   If you use a custom field in place of one of the system fields, enter the substitution: 
...
:::row:::
   ...
   > Specifies the work item types that participate in dependency tracking. From the Create a dependency dialog, you can create only those work item types listed.  
...
[  
  ...
  "Bug"  
   ...
   ```
   If you use the Scrum process, change the entry to:
...
:::row:::
   :::column span="1":::
   workItemDisplayStatesAndDisplayColors
...
:::row:::
   ...
   > Specifies the [Risk](../queries/planning-ranking-priorities.md#risk) field values. The Risk field specifies a subjective rating of the relative uncertainty around the successful completion of a user story. It's defined for the Agile process, but you can add it to work item types used in other processes. 
...
:::row:::
   ...
   > Optional configuration that specifies which Azure DevOps organizations are selectable from the Dependency dialog when creating a Cross account dependency. If you don't specify this configuration, the dialog generates a list based on previous organizations that you visited. 
   >    
...
"crossAccountDependencyToggleDefaultState": false
   ...
   ``` 
   If you don't want to create dependencies that belong to other organizations, change this configuration to: 
...
:::row:::
   ...
   > Specifies the [Priority](../queries/planning-ranking-priorities.md#priority) field values. The Priority field specifies a subjective rating of a bug, issue, task, or user story as it relates to the business. Most backlog work item types and processes define this field, but you can add it to work item types used in other processes.   
...
:::row:::
   ...
   > Specifies whether Risk functionality is enabled. If set to true, you must define the riskAssessmentValues property.
...
"Closed",  
 "Other"  
   ...
   ```
   Add or remove workflow states used in work item types that participate in dependency tracking.
...
:::row:::
   ...
   :::column-end:::
   :::column span="1":::
   Default: 8A depth of 3 corresponds to: Fabrikam/Release 1/Sprint 20. 
