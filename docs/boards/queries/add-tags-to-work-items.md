...
| Task | Example prompt |
|------|----------------|
| Find tagged items | `Find all active work items tagged with "security" in my project` |
| Add tags in bulk | `Add the tag "tech-debt" to all active bugs in the current sprint` |
| Find untagged items | `List work items with no tags assigned in the <Contoso> area path` |
| Query by multiple tags | `Find user stories tagged with both "performance" and "P1"` |
| List all tags in use | `Show all distinct tags used on work items in project <Contoso> sorted by usage count` |
| Clean up obsolete tags | `List tags in <Contoso> that are used on fewer than 2 work items` |
| Replace a tag | `Find all work items in <Contoso> tagged "wontfix" and replace that tag with "by-design"` |
| Tag by criteria | `Add tag "needs-review" to all user stories in <Contoso> that were resolved in the last week but have no linked pull request` |
| Analyze tag patterns | `Show the count of active work items per tag in area path <Contoso\\Frontend>` |
| Find tag mismatches | `List work items in <Contoso> tagged "sprint-goal" that aren't assigned to the current sprint` |
