---
title: Save your changes with Git commits | Team Services & TFS
...
such as IDE project files, build outputs, and editor autosaves. Tell Git to [ignore](ignore-files.md) stop tracking these files by setting up a `.gitignore` for your repository.
...
<p>When you commit in Visual Studio you can [push](pushing.md) the commit and [sync](pulling.md) the branch with a remote repository. 
...
> Caution: Do not amend [pushed](pushing.md) commits. If you need to update pushed code, [undo the changes with revert](undo.md) or create and push another commit.
