# [Git Documentation](index.md)

# [Overview](overview.md)
## [Learn about Git](tutorial/gitworkflow.md)

# Quickstarts
## [Create repo - Web](create-new-repo.md)
## [Create repo - CLI](share-your-code-in-git-cmdline.md)
## [Create repo - Visual Studio](share-your-code-in-git-vs.md)
## [Create repo - IntelliJ](http://java.visualstudio.com/Docs/tools/intelliJ)
## [Create repo - Xcode](share-your-code-in-git-xcode.md)
## [Create repo - Eclipse](share-your-code-in-git-eclipse.md)
## [Import repo - Web](import-git-repository.md)

# [Tutorial](gitquickstart.md)
## [1 - Create a new repo](tutorial/creatingrepo.md)
## [2 - Clone an existing repo](tutorial/clone.md)
## [3 - Save work with commits](tutorial/commits.md)
## [4 - Create work in branches](tutorial/branches.md)
## [5 - Share code with push](tutorial/pushing.md)
## [6 - Update code with fetch and pull](tutorial/pulling.md)
## [7 - Review code with pull requests](tutorial/pullrequest.md)
## [8 - Apply changes with rebase](tutorial/rebase.md)
## [9 - Copy changes with cherry-pick](tutorial/cherry-pick.md)
## [10 - Resolve merge conflicts](tutorial/merging.md)
## [11 - Undo changes](tutorial/undo.md)
## [12 - Ignore files](tutorial/ignore-files.md)
## [13 - Review history](tutorial/history.md)

# Samples
## [VSTS Samples for .NET](https://github.com/Microsoft/vsts-dotnet-samples)

# Concepts
## [History in Git](concepts/history.md)
## [History simplification](concepts/history-simplification.md)
## [Branch organization](concepts/git-branching-guidance.md)
## [Pull requests](concepts/pull-requests.md)
## [Authentication](auth-overview.md)
## [Understand dates in Git](concepts/git-dates.md)
## [Understand names in Git](concepts/git-names.md)
## IDE integration and extension
### [Visual Studio](../setup-admin/team-services/set-up-vs.md)
### [Visual Studio Code](https://marketplace.visualstudio.com/items?itemName=ms-vsts.team)
### [Eclipse](http://java.visualstudio.com/Docs/tools/eclipse)
### [IntelliJ IDEA](https://java.visualstudio.com/Docs/tools/intelliJ)
## Scale
### [Large teams](https://visualstudio.com/git-at-scale)
### [Manage large files](manage-large-files.md)

# How-to Guides
## [Authenticate with SSH](use-ssh-keys-to-authenticate.md)
## [Authenticate with the Git Credential Manager](set-up-credential-managers.md)
## [Import a repo from TFVC](import-from-TFVC.md)
## Manage repos
### [Delete a repo](delete-existing-repo.md)
### [Rename a repo](repo-rename.md)
### [Find a file](find-a-file.md)
### [Create a readme](create-a-readme.md)
### [Git and TFVC repos](team-projects.md)
## Manage branches
### [Manage your branches](manage-your-branches.md)
### [Create a new branch](create-branch.md)
### [Delete a branch](delete-branch.md)
### [Lock a branch](lock-branches.md)
### [Branch policies](branch-policies.md)
### [Branch permissions](branch-permissions.md)
## [Set up continuous integration](../build/define/triggers.md)

# Reference
## [Command reference](command-prompt.md)
## [Frequently asked questions](tutorial/howto.md)
## History
### [Commit history](commit-history.md)
### [Commit details](commit-details.md)
## Pull requests
### [Review code with pull requests](pull-requests.md)
### [Format comments with markdown](../reference/markdown-guidance.md)
### [Squash merge pull requests](merging-with-squash.md)
## [Git REST APIs](https://visualstudio.com/docs/integrate/api/git/overview)

# Resources
## [Pro Git](https://www.git-scm.com/book/en/v2)

