...
Azure DevOps analytics and reporting provide powerful insights into your development processes. With that power comes the responsibility to protect sensitive data and control access to organizational metrics. This article outlines the security concepts, access controls, and best practices for securing your analytics and reporting implementation.

## Security model overview

Analytics and reporting security uses a multilayered approach that includes:
...
- **Use personal access tokens (PATs) when necessary**: Create tokens with minimal required scopes only when Microsoft Entra ID isn't available. For more information, see [Use personal access tokens](../organizations/accounts/use-personal-access-tokens-to-authenticate.md).
::: moniker-end
...
Analytics security is based on the principle of data visibility inheritance from source projects:
...
- **Team dashboards**: Team members and project stakeholders can access these dashboards. For more information, see [Add a team, move from one default team to several teams](../organizations/settings/add-teams.md).
- **Project dashboards**: All project contributors can access these dashboards. For more information, see [Change project-level permissions](../organizations/security/change-project-level-permissions.md).
- **Personal dashboards**: Individual users keep these dashboards private.
- **Organization dashboards**: The entire organization can see these dashboards. For more information, see [Change permissions at the organization or collection-level](../organizations/security/change-organization-collection-level-permissions.md).
...
- **Manage refresh credentials**: Securely store and rotate data source credentials by using Microsoft Entra ID authentication.
- **Apply workspace permissions**: Control access to Power BI workspaces that contain Azure DevOps data.
...
When you implement analytics across multiple projects, establish clear security boundaries:
...
Manage security when you share reports with external stakeholders:
