...
| Track sprint velocity | `Write an OData query that shows completed story points per sprint for the last 6 iterations in <Contoso> project` |
| Compare team throughput | `Create an OData query that counts completed work items per week grouped by team for <Contoso> project` |
| Show cycle time distribution | `Write an OData query for Power BI that returns cycle time in days for closed user stories grouped by area path in <Contoso> project` |
