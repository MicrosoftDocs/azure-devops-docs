...
For more information, see [What is Analytics](what-is-analytics.md). It currently contains partial data. Microsoft is working to add all reportable data to Analytics. For more information, see [Data Available in Analytics](data-available-in-analytics.md).
...
- **Analytics** stores all work tracking fields, except for HTML or rich-text fields. Custom fields are automatically added to the Analytics data store.
- **Analytics** isn't a real-time store but a curated copy of data with up to a 30-second delay before changes appear.

### Migration best practices

- Always limit your query to the projects you have access to. You can generate cross-project reports by using Power BI. However, queries only return data when you have permissions to view Analytics data for all projects you query.
...
- **Analytics views**: Simplified way to create filtered datasets for Power BI reporting without writing OData queries. See [What are Analytics views?](what-are-analytics-views.md)
...
The Analytics data model consists of entity sets, whose members (entities) contain properties that you can filter, aggregate, and summarize. For an overview of supported entity sets and entity types, see [Data model for Analytics](../extend-analytics/data-model-analytics-service.md).

The following tools support interaction with the Analytics service:

- **Analytics views**: Provide a quick way to generate reports based on work tracking and test cases. You can define an Analytics view and share it with others. By using Power BI, you can build any type of report that Power BI supports. To learn more, see [What are Analytics views?](what-are-analytics-views.md)
- **Power BI**: Connect to Analytics from Power BI through an Analytics view, OData query, or OData feed. See [About Power BI integration](overview.md).
- **OData query**: Use OData to directly query Analytics from a supported browser and use the returned JSON data as needed.
- **OData metadata**: Programmatically query the Analytics metadata. See [Construct OData queries for Analytics](../analytics/analytics-query-parts.md#query-metadata).
...
For individual Analytics views, creators can set specific permissions to control who can view, edit, and delete their shared views. For detailed information about setting up and managing these permissions, see [Set permissions to access Analytics and Analytics views](analytics-security.md).

### Query performance and latency

Analytics is optimized for read and aggregation performance, and it reduces the effect that reporting scenarios have on Azure DevOps. The data copy process introduces up to a 30-second delay before the data associated with any one change shows up in Analytics. For more information, see [OData Analytics query guidelines](../extend-analytics/odata-query-guidelines.md).
