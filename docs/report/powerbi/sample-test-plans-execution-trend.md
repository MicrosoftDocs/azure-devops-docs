...
This article shows you how to get the execution state of one or more test plans in Power BI. The report you generate is similar to the following image and the Outcome trend chart of the [Progress report](../../test/progress-report.md).
...
- **NotApplicable**: Number of test cases that don't apply.
...
Use the following queries for the `TestPointHistorySnapshot` entity set to create different but similar test plan execution trend reports.
...
- `{startDate}` and `{endDate}` - Date range of interest. Enter the dates in YYYYMMDD format. For example, `20220815` for 15 August 2022.
...
:::row:::
   ...
   :::column-end:::
   :::column span="1":::
      Date range of interest. Enter the dates in `YYYYMMDD` format.
   :::column-end:::
:::row-end:::
:::row:::
   :::column span="1":::
   ...
   :::column-end:::
   :::column span="1":::
      Groups the data into bins of the same date. Produces one set of values per day in the given date range.
   :::column-end:::
:::row-end:::
:::row:::
   :::column span="1":::
...
:::row:::
   ...
   :::column-end:::
   :::column span="1":::
      While aggregating, type-casts test points with the execution outcome 'Passed' to 1 and sums them as the `Passed` metric.
   :::column-end:::
:::row-end:::
