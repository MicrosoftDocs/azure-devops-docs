...
|Project mapping|`/workItemTypes:Requirement,Task`|When you map the enterprise project plan to the project, specify requirement and task as the work item type items to participate in synchronization. You can also disallow fixed work for tasks. Fixed work is one of three types of tasks that you can use in Project. For more information, see [Change the task type Project uses to calculate task duration](../error/tf237002-cannot-open-document-excel-not-installed.md).|  
