---
title: Import and export global lists 
titleSuffix: Azure DevOps  
description: Customize or update a global list by export and import using command line tools for Azure DevOps.
...
You can define a global list in one of three ways: within its own definition file, within a definition file for a work item type, or in a global workflow file. The global list definition manages the global lists that are defined for a collection. The global list definition uses the following commands in the `witadmin` command-line tool:    
...
- `importgloballist`:  Imports global lists from an XML file. If you try to import a global list that already exists on the server, a warning asks if you want to overwrite the existing global list. If you try to import a global list that doesn't already exist, the command creates a new global list.    
...
|-------------------|---------------------|  
|`/collection`:`CollectionURL`|Specifies the URI of the project collection. For example:<br /><br /> **On-premises format:**  `http://ServerName:Port/VirtualDirectoryName/CollectionName`<br /><br /> If no virtual directory is used, then use the following format for the URI: `http://ServerName:Port/CollectionName`.|  
