...
Change the workflow for a work item type (WIT) to support your business and team processes. WITs support tracking all types of work&mdash;requirements, tasks, code defects&mdash;to support software development.  

The workflow determines the logical progression and regression of work that team members perform. It also specifies the values that appear in the drop-down menus for the State and Reason fields. For more information, see [About processes and process templates](../../boards/work-items/guidance/choose-process.md). 
...
If you're new to WIT customization, note the following: 
 
- To customize any aspect of a work item type, update its XML definition. For more information, see [All WITD XML elements reference](/previous-versions/azure/devops/reference/xml/all-witd-xml-elements-reference).   
- If you're customizing the web form that uses the new work item experience, see the [WebLayout and Control elements](/previous-versions/azure/devops/reference/xml/weblayout-xml-elements).  
- If you're customizing a client form for use with Visual Studio, see the [Layout XML element reference](/previous-versions/azure/devops/reference/xml/layout-xml-element-reference). 
...
Define the workflow by first identifying the states and the valid transitions between them. The `WORKFLOW` section of the WIT definition specifies the valid states, transitions, reasons for the transitions, and optional actions that run when a team member changes the state of a work item.  

In general, associate each state with a team member role and a task that a person in that role must perform to process the work item before changing its state. Transitions define the valid progressions and regressions between states. Reasons identify why a team member changes a work item from one state to another, and actions support automation of the transition of a work item at a point in the workflow.  

For example, set the State to **New** when a tester opens a new bug that is based on the default Agile process. The developer changes the State to **Active** when fixing the bug, and once fixed, the developer changes its state to **Resolved** and sets the value of the Reason field to **Fixed**. After verifying the fix, the tester changes the state of the bug to **Closed** and the Reason field changes to **Verified**. If the tester determined that the developer didn't fix the bug, the tester changes the state of the bug to **Active** and specifies the Reason as **Not Fixed** or **Test Failed**.  

As you design or modify a workflow, consider the following guidelines:  

-  Use the `STATE` element to define a unique state for each team member role that takes a specific action on a work item. The more states you define, the more transitions you must define. Regardless of the sequence in which you define the states, they appear in alphanumeric order in the drop-down menu for the **State** field.  

     If you add a state to a work item type that appears on the backlog or board pages in the web portal, you must also map the state to a state category. For more information, see [Workflow states and state categories](../../boards/work-items/workflow-and-state-categories.md).  

-   Use the `TRANSITION` element to define a transition for each valid progression and regression from one state to another.  

     At a minimum, define one transition for each state, and the transition from the null state to the initial state.  

     You can define only one transition from unassigned (null) to the initial state. When you save a new work item, the process automatically assigns it to the initial state.  
...
## Determine the number and types of states  
 ...
 Decide the number and types of valid states based on the number of distinct logical states in which you want the work items of that type to exist. Also, if different team members perform different actions, consider defining a state based on a member role. Each state corresponds to an action that a team member must perform on the work item to move it to the next state. For each state, define the specific actions and the team members who are allowed to perform those actions.  

 The following table provides an example of four states that track the progress of a feature and the valid users who must perform the indicated actions:  
...
You control the states to and from which team members can change a work item by defining the valid state progressions and regressions. If you don't define a transition from one state to another state, team members can't change a work item of a particular type from a particular state to another particular state.  
...
||Active (regression)|Closed in error|  
 ...
 You can restrict who is allowed to make a transition from one state to another by using the *for* and *not* attributes of the `TRANSITION` element. As the following example shows, testers can reopen a bug but developers can't.  
...
### Specify the reasons  
 ...
 When a team member changes the State field, they can keep the default reason for that transition or specify a different reason if you define additional options. Use the `DEFAULTREASON` element to specify one and only one default reason. Add extra reasons only if they help the team track or report data.  
...
- Assign a field rule under `DEFAULTREASON` or `REASON` when you want the rules to apply only for that specific reason.  
  ...
  If a field should always contain the same value, define the rule under the `FIELD` element that defines that field. For more information about rule usage, see [Rules and rule evaluation](../../organizations/settings/work/rule-reference.md).  

  Minimize the number of conditions that you define for any one type of work item. Each conditional rule adds complexity to the validation process that occurs every time a team member saves a work item. Complex rule sets might increase the time required to save the work item.  
...
###  Change the value of a field when the state changes  
 ...
 When you set the value of the **State** field for a work item to Active and save the work item, the values of the **Activated By** and **Assigned To** fields automatically set to the name of the current user. That user must be a member of the Team Foundation Server Valid Users group. The value of the **Activated Date** field is also set automatically. The following example shows the elements that enforce this rule:  
...
When you set the value of the **State** field for a work item to Active and save the work item, the `EMPTY` element automatically sets the Closed Date and Closed By fields to null and makes them read-only, as the following example shows.  
...
###  Define a field based on the contents of another field  
 ...
 When you change the value of the **State** field for a work item to Resolved and save the work item, the value of the **Resolved Reason** field is set to the value that the user specified in the **Reason** field. The following example shows the elements that enforce this rule:  
...
- [Query by assignment, workflow or board changes](../../boards/queries/query-by-workflow-changes.md)  
...
To help your users visualize the workflow, install the [State Model Visualization extension](https://marketplace.visualstudio.com/items?itemName=taavi-koosaar.StateModelVisualization) from the Visual Studio Marketplace.