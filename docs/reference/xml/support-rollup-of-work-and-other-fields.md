...
As you assign backlog items to a sprint by using the Planning pane, the sprint window updates with a running tally of the number of backlog items and tasks, as well as the **Planned Effort**. 
...
*Planned Effort* provides a sum of all *Story Points* or *Effort* values defined for backlog items assigned to the sprint. For more information, see [Assign backlog items to a sprint](../../boards/sprints/assign-work-sprint.md).

### View rollup of sprint backlogs and taskboards 

You can view a rollup of remaining work from a sprint backlog or taskboard.  

::: moniker-end 

 

From the sprint backlog, the sum of all **Remaining Work** defined for all tasks displays for the parent work item. This value also displays on the parent work item card when you view the task board.  
...
When you update the status of a task as Completed, the system automatically sets the remaining work for that task to zero. For more information, see [Task board](../../boards/sprints//task-board.md).

## Other tools that support rollup 

To get a rollup of extra data fields in Azure DevOps work tracking data, use one of the following methods:  
...
For example, the following pivot table sums the story points for user stories by area path and work item state. 
...
Export a query to Excel that contains the work items you want to provide rollup for. Then, you can write an Excel macro to get the sums and publish data back to Azure DevOps. For more information about Excel integration, see [Bulk add or modify work items with Excel](../../boards/backlogs/office/bulk-add-modify-work-items-excel.md).  
...
Several extensions from the [Marketplace](https://marketplace.visualstudio.com/vsts) provide rollup. Here are a few that might support your needs: 
...
- [TFS Aggregator](https://marketplace.visualstudio.com/items?itemName=tfsaggregatorteam.tfs-aggregator-server-plugin), supports Azure DevOps
...
You can use the [Analytics Service](../../report/powerbi/what-is-analytics.md) to answer quantitative questions about your projects. By using this service, you can add [Analytics widgets](../../report/dashboards/analytics-widgets.md) to your dashboard. Or, you can create additional reports by using [Power BI](../../report/powerbi/overview.md).  
...
## SQL Server Reporting Services report  
 ...
 Several out-of-the-box reports provide rollup. Here's an example of rollup of completed and remaining work that the Stories Overview report provides. This report is part of the default Agile process template.  
  
 ![Stories Overview example report](media/procguid_agilereports.png "ProcGuid_AgileReports")  
  
 If you deploy SQL Server Analysis Services, you can get rollup for backlog items from these reports. The refresh frequency for these reports is two hours.  
...
To add reports to your on-premises Azure DevOps Server deployment, see [Add reports to a project](/previous-versions/azure/devops/report/admin/add-reports-to-a-team-project).  
...
- Use parent-child links to link work items that contain values you want to roll up.  
  
- Add required fields to the WITs that capture the rollup values. The default fields used to schedule work are only present on the task work item. These fields are:  
  ...
  - Completed Work (Microsoft.VSTS.Scheduling.CompletedWork): The amount of work spent implementing a task. (Agile and CMMI)   
  - Remaining Work (Microsoft.VSTS.Scheduling.RemainingWork): This field supports burndown charts.  
  
    If you create your project by using the Visual Studio Scrum process template, only Remaining Work is defined in the task.  
 ...
 **A:** No. The data you enter for team capacity isn't stored in the regular data stores.