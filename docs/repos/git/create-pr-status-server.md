...
9. Build the status object to post on the PR. 
   ...
   `State` is an enum of type [GitStatusState](/rest/api/azure/devops/git/pull%20request%20statuses/get#gitstatusstate). Use `succeeded` to indicate that the PR has passed the status check and is ready to merge. 
