...
To link work items to a new PR:
  ...
  1. Start to enter a work item ID or title, and select the work item to link from the dropdown list that appears. Search by ttile returns work items filtered by state, all work items with states having state categories Completed" and "Removed" are excluded. These work items are also filtered by date and user: only created or updated in the last 30 days ones are shown, also they should be created by, assigned to, or authorized as the current user
