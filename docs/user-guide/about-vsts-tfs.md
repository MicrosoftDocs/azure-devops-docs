...
Team Foundation Server is the on-premises offering that's built on a SQL Server back end. Enterprises usually choose on-premises TFS when they need their data to stay within their network or when they want access to SharePoint sites and SQL Server reporting services that integrate with TFS data and tools.  
...
* You currently have to administer organizations one at a time, a process that can be cumbersome when you have many organizations. We're working to support enterprise-wide policies.

### Authentication

With TFS, you usually connect to an intranet server (for example, ```https://tfs.corp.contoso.com:8080/tfs```). You authenticate with Windows Authentication and your Azure Active Directory (Azure AD) domain credentials. This process is usually transparent, and you never see any kind of sign-in experience.
...
In TFS, you provide users access to deployments by adding Azure AD groups to various TFS groups 
(for example, the Project Contributors group for an individual  project). The Azure AD group memberships are kept in sync. 
As users are added and removed in Azure AD, they also gain and lose access to TFS. 
...
In TFS and Azure DevOps Services, you can give free access to work item features to an unlimited number of stakeholders. Also, an unlimited number of Visual Studio subscribers can have access to all Basic features at no additional charge. You need to pay only for other users who need access.
...
in the two products. Some TFS features are not supported in Azure DevOps Services at all. For example, Azure DevOps Services does not support integration with SharePoint or Project Server.  
...
To help you avoid these issues in Azure DevOps Services, custom process templates and **witadmin.exe** have always been disabled (only export functions are enabled). This approach has 
...
* A [PowerBI connector](../report/powerbi/overview.md), which provides a combination of simplicity and power. We plan to make it available in TFS in a future release.
* [Analytics](/azure/devops/report/analytics/), the reporting platform for Azure DevOps Services that answers quantitative questions about the past or present state of your projects. This service provides a concise data model over the Azure DevOps Services suite. Built for reporting, the Analytics Service is optimized for fast read-access and server-based aggregations. We plan to make it available in TFS in a future release.
