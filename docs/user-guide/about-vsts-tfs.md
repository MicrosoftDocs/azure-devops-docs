...
Team Foundation Server is the on-premises offering built on a SQL Server backend. Enterprises typically choose on-premises TFS when they need their data to stay within your network, or they want access to SharePoint sites and SQL Server reporting services that integrate with TFS data and tools.  

While both offerings provide the same [essential services](services.md), compared with TFS, VSTS provides these added benefits:
...
Use this topic to determine which offering&mdash;cloud or on-premises&mdash;meets your needs by considering these important areas:
...
We are planning a third option for scoping and scaling data in VSTS&mdash;a new entity. Rather than adding support for multiple team project collections within an enterprise, multiple
organizations could be grouped within an enterprise. Additionally, we will merge organizations and their single
team project collections into a single entity. The organization will be similar to the TFS deployment,
and the enterprise will be similar to the TFS collection.
...
[Azure Active Directory)](https://docs.microsoft.com/azure/active-directory/active-directory-whatis) account 
credentials, depending on your VSTS organization setup. You can also set up Azure Active Directory 
to require features like multi-factor-authentication, IP address restrictions, and so on.

We recommend that organizations configure their VSTS organizations to use Azure Active Directory rather than 
...
by adding Azure Active Directory groups to TFS groups. If you use Microsoft accounts instead of Azure Active Directory, you will have to
...
If you use Azure Active Directory groups to provide access to groups of users, VSTS will assign appropriate 
