...
Do you want more disk space for your agents? Azure Managed DevOps Pools supports attaching an empty data disk to the agents in your Managed DevOps pool. When you attach a data disk, you can get more storage space without incurring the potentially greater cost of moving your virtual machine (VM) size to a more expensive size that has more built-in storage.
...
1. Select your Managed DevOps pool from the list.
...
Configure the following properties for your disk, and select **Apply** (or **Next** if you're creating a pool) to save your changes. You can add a single empty data disk configuration for your Managed DevOps pool. All agent instances have an empty data disk attached that matches the configuration you specify.

To delete the data disk configuration for an existing pool, select **Delete**.

#### [ARM template](#tab/arm/)

Additional disk storage is configured in an Azure Resource Manager template (ARM template) in the property `dataDisks` section under `storageProfile`. In the following example, a 10-GB `Standard_LRS` disk is configured with the drive letter `Q` and no caching.
