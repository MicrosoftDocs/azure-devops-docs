...
1. In the confirmation window, create the identity by selecting **Create**.

1. Go to your pool in the Azure portal, and then select **Settings** > **Identity** > **Add**.

   :::image type="content" source="./media/configure-identity/add-identity-button.png" alt-text="Screenshot that shows the button for adding an identity.":::
...
You can use Managed DevOps Pools to fetch certificates from an Azure Key Vault instance during agent provisioning. The certificate already exists on the machine by the time it runs your Azure DevOps pipelines. To use this feature, add an identity to your pool as shown in the previous example, and then assign the **Key Vault Secrets User** role to the identity.
