...
You can view the status of the agents in your Managed DevOps pool on the **Agents** pane. You can use this information to find out how many agents are running jobs and see how many standby agents are online.

## Agents list

To view the status of the agents in your pool, go to your Managed DevOps pool and select **Settings** > **Agents**.
...
| Status | Description |
|--------|-------------|
| **Ready**  | The agent is online and is ready to accept jobs. You see this status if you have standby agents configured or stateful pools with a grace period. |
| **NotReady** | A stateful agent job is complete, but the agent isn't ready to accept a new job. |
| **Allocated** | The agent is running a job. |
| **PendingReimage** | The agent job is complete and the agent is ready to be reimaged. This status is typical for stateless agents with **Standby agent mode** enabled. |
