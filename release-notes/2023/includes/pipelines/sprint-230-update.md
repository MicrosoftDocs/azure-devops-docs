...
Azure Pipelines has a number of tasks that have been deprecated. Deprecated tasks will be retired early 2024. To help customers identify pipelines that are using deprecated tasks, pipeline will show warnings if such a task is used. We have also updated the [Task Reference](/azure/devops/pipelines/tasks/reference/?view=azure-pipelines&preserve-view=true) to more clearly convey deprecation status and retirement date. The Task reference also includes alternatives (e.g. a newer major version) to the task.
