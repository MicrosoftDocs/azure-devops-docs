...
In YAML pipelines, checks are used to control the execution of stages on [protected resources](/azure/devops/pipelines/security/resources?view=azure-devops). One of the common checks that you can use is an [exclusive lock check](/azure/devops/pipelines/process/approvals?view=azure-devops&tabs=check-pass#exclusive-lock). This check lets only a single run from the pipeline proceed. When multiple runs attempt to deploy to an environment at the same time, the check cancels all the old runs and permits the latest run to be deployed.
...
Azure Pipelines supports two sets of tasks for publishing/downloading artifacts. [PublishPipelineArtifact and DownloadPipelineArtifact](/azure/devops/pipelines/artifacts/pipeline-artifacts?view=azure-devops&tabs=yaml) are the newer and recommended tasks for performing these steps.
