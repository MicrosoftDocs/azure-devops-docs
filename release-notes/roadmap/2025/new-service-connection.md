...
To improve the security in these scenarios, a new service connection type called to connect to Azure DevOps is being introduced. It uses an Workload identity (managed identity or service principal) that can be added as a user in Azure DevOps with specific permissions. This allows you to authenticate to resources from a pipeline task using this service connection and restrict access to specific pipelines.